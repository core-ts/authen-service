...
function useAuthenticator(status, check, account, repository, getPrivileges, lockedMinutes, maxPasswordFailed, send, expires, codeRepository, compare, hash, hasTwoFactors, gen) {
  return new Authenticator(status, compare, account, repository, getPrivileges, lockedMinutes, maxPasswordFailed, send, expires, codeRepository, hash, hasTwoFactors, gen, check);
}
...
var Authenticator = (function () {
  function Authenticator(status, compare, account, repository, getPrivileges, lockedMinutes, maxPasswordFailed, send, expires, codeRepository, hash, hasTwoFactors, gen, check) {
    this.status = status;
  ...
  Authenticator.prototype.authenticate = function (info) {
    return __awaiter(this, void 0, void 0, function () {
      var s, result, username, password, account0, user, tnow, valid, lockedUntilTime, lockedUntilTime, locked, lockedUntilTime, locked, passwordExpiredTime, contact, twoFactors, sentCode, savedCode, codeExpired, res0, code, validPasscode, account, privileges;
      return __generator(this, function (_a) {
        switch (_a.label) {
          ...
          case 7:
            ...
            if (!(contact && this.hash && this.expires && this.expires > 0 && this.codeRepository && this.send && this.compare)) return [3, 20];
            twoFactors = user.twoFactors;
            if (!(!twoFactors && this.hasTwoFactors)) return [3, 9];
            return [4, this.hasTwoFactors(user.id)];
          case 8:
            twoFactors = _a.sent();
            _a.label = 9;
          case 9:
            if (!twoFactors) return [3, 20];
            if (!(!info.step || info.step <= 1)) return [3, 15];
          ...
          case 17:
            ...
            if (!!validPasscode) return [3, 18];
            return [2, { status: this.status.fail }];
          case 18: return [4, this.codeRepository.delete(user.id)];
          case 19:
            _a.sent();
            _a.label = 20;
          case 20:
            ...
            if (!this.getPrivileges) return [3, 22];
...
var SqlUserRepository = (function () {
  ...
  SqlUserRepository.prototype.pass = function (userId, deactivated) {
    ...
    if (!deactivated || !this.status || c.status.length === 0) {
      ...
      return this.db.execute(stmt.query, stmt.params).then(function (v) { return (v > 0 ? true : false); });
    }
    else {
      ...
      if (activated && activated !== "") {
        if (c.user === c.password || c.password === undefined) {
          ...
          return this.db.executeBatch([stmt1, stmt2], true).then(function (v) { return (v > 0 ? true : false); });
        }
      }
      else {
        var stmt = buildUpdatePassword(pass, this.db.param, this.password, this.id, userId);
  ...
  SqlUserRepository.prototype.fail = function (userId, failCount, lockedUntilTime) {
    ...
    params.push(userId);
    return this.db.execute(query, params).then(function (v) { return (v > 0 ? true : false); });
  };
...
function generate() {
  var n = crypto_1.randomInt(1, 999999);
...
var MailSender = (function () {
  ...
  MailSender.prototype.send = function (to, passcode, expireAt) {
    ...
    var body = util_1.format.apply(void 0, __spreadArrays([this.body], [passcode, diff]));
    var msg = {
      to: to,
      from: this.from,
